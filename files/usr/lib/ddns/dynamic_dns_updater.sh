...
if [ -n "$ip_device" ]; then
	sanitize_variable ip_device "[0-9A-Fa-f:]" ""
	printf "%s" "$ip_device" | grep -Eq "^$MAC_REGEX$" || write_log 14 "Service section not configured correctly! Invalid 'ip_device'"
	ip_device=$(printf "%s" "$ip_device" | tr 'A-F' 'a-f')
fi
...
if [ "$ip_source" = "device" ]; then
	[ "$use_ipv6" -eq 1 ] || write_log 14 "Service section not configured correctly! 'device' source requires 'use_ipv6' enabled"
	[ -n "$ip_device" ] || write_log 14 "Service section not configured correctly! Missing 'ip_device'"
...
while : ; do
	...
	if [ $CURR_TIME -ge $NEXT_TIME -o "$CURRENT_IP" != "$REGISTERED_IP" ]; then
		...
		elif [ $ERR_LAST -eq 128 ]; then
			write_log 5 "Update skipped - waiting for public address"
