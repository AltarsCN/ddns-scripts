...
if [ -n "$param_opt" ]; then
	for pair in $param_opt; do
		...
		dns_record_id=*)
			...
			;;
		api_token=*|zone_api_token=*)
			api_token=${pair#*=}
			if [ -n "$api_token" ]; then
				write_log 7 "Cloudflare API token provided via param_opt"
			fi
			;;
		*)
			# Ignore others
...
if [ $__USE_TOKEN -eq 1 ]; then
	[ -n "$__TOKEN" ] || write_log 14 "Service section not configured correctly! Missing Cloudflare API token"
else
	[ -n "$username" ] || write_log 14 "Service section not configured correctly! Missing key as 'username'"
	[ -n "$password" ] || write_log 14 "Service section not configured correctly! Missing secret as 'password'"
fi
...
# set headers
if [ $__USE_TOKEN -eq 1 ]; then
	write_log 7 "Using Cloudflare API token authentication"
